...
from essentialpipeline.app.middleware import project_access_required
...
def get_project(project_id):
    ...
    return jsonify(g.project.to_dict())
...
def publish_version(project_id, version_id):
    ...
    if version.project_id != project_id:
        return jsonify({'error': 'Version does not belong to this project'}), 400

    if version.is_published:
        return jsonify({'error': f'Version {version.version} is already published'}), 409
    ...
    if not gate_result['passed']:
        return jsonify({'error': 'Publish blocked by governance gate', 'checks': gate_result['checks']}), 403
